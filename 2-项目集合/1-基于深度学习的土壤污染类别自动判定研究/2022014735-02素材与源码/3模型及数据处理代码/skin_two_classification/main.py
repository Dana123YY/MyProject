...
def use(img_path):
    ...
    if args.model_path:
        # 加载模型
        model.load_state_dict(torch.load("./checkpoints/model_cur.pth", map_location='cpu'))
    img = transform(Image.open(img_path))
    # evaluate
    with torch.no_grad():
        model.eval()  # 设置成eval模式
        inputs= torch.autograd.Variable(torch.unsqueeze(img, dim=0).float(), requires_grad=False)
        # compute output
        print(inputs.shape)
        outputs = model(inputs)  # (16,2)
        # dim=1 表示按行计算 即对每一行进行softmax
...
if __name__ == "__main__":
    ...
    # else:
    #     test(use_cuda)
    use(r'C:\Users\86199\Desktop\soil_image_spider\classification_model\used_dataset\neg0001.jpg')